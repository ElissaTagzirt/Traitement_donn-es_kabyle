...
def process_pdf(pdf_path, output_md, header_cutoff=100, col_split_x=None):
    ...
    with pdfplumber.open(pdf_path) as pdf, open(output_md, "w", encoding="utf-8") as md_file:
        ...
        for i, page in enumerate(pdf.pages):
            ...
            # Extraction des tableaux avec pdfplumber (si besoin, on pourrait utiliser Camelot ici)
...
def process_pdf_worker(args):
    ...
    """
    pdf_path, output_md, header_cutoff, col_split_x = args
    print(f"Processing {pdf_path} => {output_md}")
    process_pdf(pdf_path, output_md, header_cutoff, col_split_x)

def main():
    PDF_INPUT_DIR = "./PDFs/non_imp/with_tables"         # Répertoire contenant vos PDF
    MD_OUTPUT_DIR = "./Markdown_non/with_tables"          # Répertoire de sortie pour les fichiers .md
    ...
    for pdf_name in pdf_files:
        ...
        tasks.append((pdf_path, output_md, header_cutoff, col_split_x))
    
    # Utilisation de tous les cœurs disponibles
    n_workers = cpu_count()
    print(f"Utilisation de {n_workers} cœurs pour le traitement.")
